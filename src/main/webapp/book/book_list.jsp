	...
	<FORM action="bookSearch.ao">
      ...
      <button type="submit">검색</button>    
		...
		<table>
			...
			%>
			<tr>
				<td style="padding:0px 10px"><%=articleList.get(i).getBookno()%></td>
				<td style="padding:0px 10px">
					<a href="bookDetail.ao?no=<%=articleList.get(i).getNo()%>&page=<%=nowPage%>&bsign=<%=articleList.get(i).getBsign()%>">
						<%=articleList.get(i).getTitle()%>
					</a>
				</td>
				<td align=center><%=articleList.get(i).getAutho()%></td>
		...
		%>
		<a href="bookList.ao?page=<%=nowPage - 1%>">[이전]</a>&nbsp;
		<%
		}
		%>
		...
		%>
		<a href="bookList.ao?page=<%=nowPage + 1%>">[다음]</a>
		<%
		}
		%>
		</td></tr>
		...
		<%} else if(bookList != null) { %>
			...
			<tr>
				...
				<td style="padding:0px 10px">
					<a href="bookDetail.ao?no=<%=bookList.get(i).getNo()%>&page=<%=nowPage%>">
			...
			%>
			<tr><td colspan="5" align=center>
			<a href="<%=request.getContextPath() %>/MainU.jsp">[홈으로]</a>&nbsp;
		<%
		if (nowPage <= 1) {
		%>
		[이전]&nbsp;

	...
	<section id="articleForm">
		...
		<table>
            ...
            <th colspan=2>
               ...
               <a href="borrowList.ao?id=<%=session.getAttribute("id")%>&page=${page }"> [대출목록] </a>&nbsp;&nbsp;
               <a href="bookReturn.ao?id=<%=session.getAttribute("id")%>&no=<%=article.getNo()%>&b_pass=<%=request.getParameter("b_pass")%>"> [반납하기] </a>&nbsp;&nbsp;
               <a href="borrowExtend.ao?id=<%=session.getAttribute("id")%>&no=<%=article.getNo()%>"id='borrowExtend'>[연장하기]</a>
            </th>
         </tr>
		</table>
	</section>
